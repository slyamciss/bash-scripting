...
# Meaning special variables for inputs are different for function from main program
# 4. Cases we need to come back immediately from function to main program then in that case we use return command.
# return command is like exit command, where exit exits the script where as return with exits the function.